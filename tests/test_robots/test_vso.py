...
def test_mapping_ends_with_unsupported_voltage_command(monkeypatch, tmp_path):
    """
    Documents a real bug: _create_linear_joint_mapping ends by switching to VOLTAGE
    mode and calling set_motor_voltage(0.0), copy-pasted from osl.py. MaxonActuator
    has VOLTAGE=None in its mode configs and inherits set_motor_voltage from
    PositionControlActuatorBase, which raises UnsupportedControlModeError. The mode
    switch itself succeeds, so nothing gates the call -- with a real MaxonActuator
    this method always raises at the very end, after the map is already saved.

    RecordingActuator tolerates the call, so this test pins the current behaviour.
    When the driver is fixed (set_motor_pwm(0.0) or stop()), this test should be
    updated to assert the new call instead.
    """
    monkeypatch.chdir(tmp_path)
    actuator = RecordingActuator("ankle", 10, _motor_constants())
    actuator._is_homed = True
    ...
    assert ("set_motor_voltage", 0.0) in actuator.calls
    assert actuator.mode == CONTROL_MODES.VOLTAGE
