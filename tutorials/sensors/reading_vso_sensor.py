...
def read_sensors(data_logger: Logger) -> None:
    ...
    with vso, data_logger:
        if vso.sensors.get("adc") is None:
        ...
        if vso.sensors.get("hallEffect_sensor") is not None:
            hall = vso.sensors["hallEffect_sensor"]
            hall.configure()
        ...
        calib_offset = 0.0
        if vso.sensors.get("ankle_encoder") is not None:
            ankle_sensor = vso.sensors["ankle_encoder"]
